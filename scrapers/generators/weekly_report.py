...
class WeeklyReportGenerator:
    ...
    def generate(self, items: List[ScrapedItem], week_id: str,
                 manual_notes: dict = None) -> str:
        """生成周报Markdown文件"""
        # 分类
        weixin_items = [i for i in items if i.source == "sogou_weixin"]
        podcast_items = [i for i in items if i.source == "apple_podcasts"]
        bili_items = [i for i in items if i.source == "bilibili"]
        ...
        # 综合TOP（所有来源混合）
        top_items = sorted(items, key=lambda x: x.extra.get("score", 0), reverse=True)
        ...
        entities_raw = extractor.extract(items)
